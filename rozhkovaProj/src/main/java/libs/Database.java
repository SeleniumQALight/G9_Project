...
public class Database {
    ...
    public Database(String dbDriver, String bdUrl, String user_name, String user_pass)
        ...
        // Load driver for JDBC class. передавати шлях до нашої бази даних. якщо їх декілька
     ...
     * String
     * якщо ми хочемо витягнути тільке одне значення з бази даних
     */
     ...
     *  as List of Strings
     * якщо ми хочемо витягнути всі значення з бази даних. але повертає 1 рядочок
     */
     ...
     * якщо ми хочемо витягнути всі значення з бази даних. повертає всі рядочки. універсальний. але повертає список списків. вся таблиця
     */
     ...
     * returns result set as List<Map>
        * якщо ми хочемо витягнути всі значення з бази даних. повертає всі рядочки. універсальніший і зрозуміліший. повертає список мап (ключик:значення), працювати простіше і можна витягати значення. вся таблиця
     */
    public ArrayList<Map<String, String>> selectTableAsMap(String query) throws SQLException {
        // Create statement for connection, execute query and save outcome in ResultSet. встановлення короткого зв'язку
        Statement stm = connection.createStatement();
        //System.out.println(query); список даниих і метаінформація
        ResultSet rSet = stm.executeQuery(query);

        // Get ResultSet's meta data. скільки колонок прийшло
        ResultSetMetaData meta = rSet.getMetaData();
        int columnNumber = meta.getColumnCount();
//ств. змінну з данними
        ArrayList<Map<String, String>> resultTable = new ArrayList<>();

        while (rSet.next()) {               //next() - переходить на наступний рядок. якщо є наступний рядок, то він повертає true
            ...
            for (int k = 1; k < (columnNumber + 1); k++) {
                ...
                value = value.trim();  //видаляємо пробіли, бо бд може повертати з пробілами
                resultSet.put(meta.getColumnName(k), value); //записати пару ключ-значення в нашу мапу
            }
